...
var ContextVssal = (function() {
    ...
    ContextVssal = function(config) {
        ...
        var hasCode = this._getQueryString('code');
        if (this.config.auto || hasCode || !this.getCachedToken()) {
            this.acquireToken(hasCode);
        }
    };
    ...
    ContextVssal.prototype.getCachedToken = function() {
        var token = this._getItem(this.CONSTANTS.STORAGE.ACCESS_TOKEN);
    ...
    ContextVssal.prototype.getCachedUser = function() {
        ...
        this._user = this._createUser(username);
        return this._user;
    };
    ...
    ContextVssal.prototype._newToken = function(callback) {
        ...
        this.info('newToken is called for resource');
        var request = ajax({
            method: 'post',
            ...
            data: {
                ...
                assertion: this._getQueryString('code'),
                redirect_uri: this.config.redirectUri
            }
        }).always(function(response) {
            self.verbose('Renew token Expected ');
     ...
     */
    ContextVssal.prototype.acquireToken = function(hasCode) {
        var token = this.getCachedToken();
        if (token) {
            this.info('Token is already in cache');
            return token;
        } else if (hasCode) {
            this._newToken();
        } else {
    ...
    ContextVssal.prototype.logOut = function() {
        ...
        if (this.config.logOutUri) {
            ...
            var logout = '';
            if (this.config.redirectUri) {
                logout = 'redirectUrl=' + encodeURIComponent(this.config.redirectUri);
            }
     ...
     * @ignore
     */
    ContextVssal.prototype._createUser = function(username) {
        var user = null;
        var parsedJson = this._extractToken(username);
        if (parsedJson && parsedJson.hasOwnProperty('aud')) {
            if (parsedJson.aud.toLowerCase() === this.config.clientId.toLowerCase()) {
                ...
                if (parsedJson.hasOwnProperty('email')) {
                    user.userName = parsedJson.email;
                }
            } else {
                this.warn('username has invalid aud field');
            }
    ...
    ContextVssal.prototype.saveToken = function(requestInfo) {
        ...
        this._saveItem(this.CONSTANTS.STORAGE.ERROR_DESCRIPTION, '');
        // Record error 
        if (requestInfo.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
            this.info('Error :' + requestInfo[this.CONSTANTS.ERROR] + '; Error description:' + requestInfo[this.CONSTANTS.ERROR_DESCRIPTION]);
            this._saveItem(this.CONSTANTS.STORAGE.ERROR, requestInfo[this.CONSTANTS.ERROR]);
            this._saveItem(this.CONSTANTS.STORAGE.ERROR_DESCRIPTION, requestInfo[this.CONSTANTS.ERROR_DESCRIPTION]);

            this._saveItem(this.CONSTANTS.STORAGE.SCOPE, '');
            this._saveItem(this.CONSTANTS.STORAGE.ACCESS_TOKEN, '');
            this._saveItem(this.CONSTANTS.STORAGE.EXPIRATION_KEY, 0);
        }
        if (requestInfo.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
            this.info('Fragment has access token');
            // save token with related resource refresh_token
            this._saveItem(this.CONSTANTS.STORAGE.SCOPE, requestInfo[this.CONSTANTS.SCOPE]);
            this._saveItem(this.CONSTANTS.STORAGE.ACCESS_TOKEN, requestInfo[this.CONSTANTS.ACCESS_TOKEN]);
            this._saveItem(this.CONSTANTS.STORAGE.EXPIRATION_KEY, this._expiresIn(requestInfo[this.CONSTANTS.EXPIRES_IN]));
        }
