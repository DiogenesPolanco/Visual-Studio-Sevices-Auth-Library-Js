...
var ContextVssal = (function() {
    ...
    ContextVssal.prototype._createUser = function(username) {
        ...
        var request = ajax({
            ...
            headers: {
                ...
                'Authorization': 'Bearer ' + self.getCachedToken()
            }
    ...
    ContextVssal.prototype.isCallback = function(hash) {
        hash = this._getHash(hash === undefined ? window.location.hash : hash);
        var parameters = this._deserialize(hash);
        if (hash === "") {
            parameters = { code: this._getQueryString('code') };
        }
        return (
            parameters.hasOwnProperty(this.CONSTANTS.CODE) &&
            !this.getCachedToken()
        );
    };
