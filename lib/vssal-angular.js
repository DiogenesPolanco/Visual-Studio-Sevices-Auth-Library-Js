...
var ContextVssal = (function() {
    ...
    ContextVssal.prototype._base64ToArrayBuffer = function(base64) {
        ...
        vssalModule.provider('vssalAuthenticationService', function() {
            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', '$injector', function($rootScope, $window, $q, $location, $timeout, $injector) {
                ...
                return {
                    ...
                    base64ToArrayBuffer: _vssal._base64ToArrayBuffer,
                    userInfo: _oauthData,
                    ...
                    getUser: function() {
                        ...
                        _vssal.getCachedUser().then(function(CachedUser) {
                            if (CachedUser) {
                                ...
                                _vssal.error('Error when getting user');
        ...
        vssalModule.provider('vssalVisualStudioService', function() {
            ...
            this.$get = ['$rootScope', '$window', '$q', '$http', '$location', '$timeout', '$injector', 'vssalAuthenticationService', function($rootScope, $window, $q, $http, $location, $timeout, $injector, vssalAuthenticationService) {
                ...
                return {
                    ...
                    UploadAttachment: function(filename, base64contents) {
                        return vssalAuthenticationService.promise({
                            headers: {
                            ...
                            AutoVSO: true,
                            method: 'POST',
                            data: vssalAuthenticationService.base64ToArrayBuffer(base64contents),
                            url: vssalAuthenticationService.getUrlResourceTenant() + 'DefaultCollection/_apis/wit/attachments?filename=' + filename + '&api-version=1.0'
                        });
                    },
                    CreateWorkItem: function(workItem) {
                        ...
                        if (workItem.type == "Bug") {
                            ...
                            if (workItem.attachment)
                                patchDocument.push({
                                    ...
                                    value: {
                                        rel: "AttachedFile",
                                        url: workItem.attachment.url,
                                        attributes: {
                                            comment: workItem.attachment.comment
                                        }
                                    }
                                });
                        }
                        return vssalAuthenticationService.promise({
                            ...
                            AutoVSO: true,
                            method: 'PATCH',
                            data: JSON.stringify(patchDocument),
                            url: vssalAuthenticationService.getUrlResourceTenant() + 'DefaultCollection/' + workItem.project + '/_apis/wit/workitems/$' + workItem.type + '?api-version=2.2'
                        });
                    }
                };
            }];
